...
class World:
    def __init__(self, u_list=None, p_list=None, visualize=True):
        ...
        # [ADDED]: Add some additional objects to the scene: ######################################
        # Add box object (box.urdf)
        box_id = p.loadURDF(os.path.join(file_path,"box.urdf"), [0, 0, 0.05])
        p.changeDynamics(box_id, -1, lateralFriction=0.9)
